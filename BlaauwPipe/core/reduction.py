...
class Reduction(Plugin):
    def __init__(self):
        ...
        self.description = """ Core plugin that uses the previously generated master correction frames
                           ...
                           """
        self.description_short = "Reduce all the present light frames"

    def on_run(self, obs, plp):
        self.reduce_imgs(obs, plp)

    def on_run_single(self, obs, plp, file, max_days_off=365):
        self.reduce_img(self, obs, plp, file, max_days_off=max_days_off) 
            
    @staticmethod
    def reduce_img(obs, plp, light_file, max_days_off):
...
#                                  (folder_datetime + timedelta(days=max_off)).date(), light_file])
            pending_json = {
                ...
                "DARK-AGE": dark_off,
                "FLAT-AGE": flat_off,
                "Expires": (folder_datetime + timedelta(days=max_off)).date(),
                "Path": light_file
            }
            Pending.append_pending_log(pending_json)
            failed_reds += 1
            ps.updateWarning(f"""Reduced light file with non-zero days-off ({max_off})
             saved at {BlaauwPipe.strip_filepath(savepath)}""", progressbar=True)
        else:
    ...
    #     cr_time_scl = cr_time_rel / time_diff
        ...
        for light_file in lightFiles:
            ...
            success = self.reduce_img(obs, plp, light_file, max_days_off=365)
            if not success: failed_reds += 1
        ps.updateDone(f"Reduction process finished", progressbar=True)
            
        if len(lightFiles) - failed_reds > 0:
            ps.done(f"Succesfully reduced {len(lightFiles) - failed_reds} light files!")
        if failed_reds > 0:
            ps.warning(f"{failed_reds} light files were not optimally reduced!")
