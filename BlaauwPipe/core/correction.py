...
from core.pending import Pending
...
class Calibration(Plugin):
    def __init__(self):
        super().__init__()
        self.title = "The Calibration-Creator"
        self.call_level = 200
        self.command = "-c"
        self.command_full = "--calibration"
    ...
    def on_run(self, obs, plp):
        self.create_calibrations(obs, plp)
        
    def create_calibrations(self, obs, plp):
        """ Function that generates and saves all possible master calibration 
        ...
        ps.done(f"Successfully created master calibration frames!")
    ...
    def create_mdark(obs, plp, d_cluster, index, max_days_off=365):
        ...
        try:
            ...
            ps.updateFailed(f"Master dark creation failed: {err} for {plp.working_dir}")
            ...
            pending_json = {
                "Date": dark_creation.date().strftime("%d-%m-%Y"),
                "Frame type": "Dark file",
                "Binning": binning,
                "Filter": fltr,
                "BIAS-AGE": -1,
                "DARK-AGE": -1,
            ...
            Pending.append_pending_log(pending_json)
            return
        ...
        if max_off > 0:
            ...
            pending_json = {
                "Date": folder_datetime.date().strftime("%d-%m-%Y"),
                "Frame type": "Dark file",
                "Binning": binning,
                "Filter": fltr,
                "BIAS-AGE": bias_off,
                "DARK-AGE": -1,
                "FLAT-AGE": -1,
                "Expires": (folder_datetime + timedelta(days=max_off)).date().strftime("%d-%m-%Y"),
                "Path": savepath
            }
            Pending.append_pending_log(pending_json)
    ...
    def create_mflat(obs, plp, f_cluster, fltr, index, max_days_off=365):
        ...
        try:
            ...
            ps.updateFailed(f"Master flat creation failed: {err} for {plp.working_dir}")
            logging.warn(f"SuitableMasterMissingError: could not find frames for flat cluster {index+1} of filter {fltr}")

            # Let's hope the pending log can someday fix this 
            pending_json = {
                "Date": flat_creation.date().strftime("%d-%m-%Y"),
                "Frame type": "Flat file",
                "Binning": binning,
                "Filter": fltr,
                "BIAS-AGE": -1,
                "DARK-AGE": -1,
    ...
    def recreate_mdark(obs, plp, file_path, binning, fltr):
        ...
        # Handle the master dark
        d_clusters = plp.get_dark_clusters(binning)
        BlaauwPipe.create_mdark(obs, plp, d_clusters[cluster_index], cluster_index)
    ...
    def recreate_mflat(obs, plp, file_path, binning, fltr):
        ...
        f_clusters = plp.get_flat_clusters(binning, fltr)
        BlaauwPipe.create_mflat(obs, plp, f_clusters[cluster_index], fltr, cluster_index)
