...
class BlaauwPipe(object):
    ...
    def load_args(self):
        ...
        for plugin in self.core_plugins:
            parser.add_argument(plugin.command, plugin.command_full, action="store_true", help=plugin.description_short)
...
#         parser.add_argument("-m", "--modules", action="store_true", help="Run external modules stored in /plugins")
    ...
    def check_args(self):
        ...
        if args.folder:
            ...
            targets = BlaauwPipe.get_targets_from_folder(args.folder)
...
#                 args.pending or args.astrometry or args.modules):
        if not True in [getattr(args, plugin.command_full.replace("--",""), False) for plugin in self.plugins]:
    ...
    def check_obs(self):
        ...
        needed_dirs = [os.path.join(self.working_dir, cst.raw_dir),
                       os.path.join(self.working_dir, cst.correction_dir),
                       os.path.join(self.working_dir, cst.reduced_dir)]
    ...
    def print_plugins(self):
        ps.done("The following plugins are initialized:")
        for plugin in self.plugins:
            ps.done(f"  {plugin.title} ({plugin.call_level})")
    ...
    def get_targets_from_folder(folder):
        ...
        if folder.endswith(cst.tele_subsubdirs):
            ...
            ps.failed("Foldertype not understood")
...
#         self.plugins = [plugin for plugin in self.core_plugins if plugin.command_full in arg_plugins]
        self.plugins = self.core_plugins + self.extra_plugins
...
#             [self.plugins.append(plugin) for plugin in self.extra_plugins]
    ...
    def run_plugins(self):
        # Execute each plugin
        for plugin in self.plugins:
            print(plugin.title, plugin.command_full)
            if "pending" in plugin.command_full:
                self.rerun_pending(self.obs, self.plp)
            elif getattr(self.args, plugin.command_full.replace("--",""), False):
    ...
    @staticmethod
    def append_pending_log(self, pending_json):
    ...
    def rerun_pending(self, obs, plp):
        ...
        folder_date = obs.foldername.replace(cst.tele_path, '').split(os.sep)[1]
    ...
    #         date, frame_type, binning, fltr, bias_off, dark_off, flat_off, expiry, file_path = line
            ...
            if pending_json["Frame type"] == "Light file":
                ...
                for plugin in self.core_plugins:
                    if "reduce" in plugin.command_full:
                        plugin.reduce_img(obs, plp, pending_json["Path"], max_days_off=max_days_off)
                        break
            ...
            # Re-reduce dark frame
                ...
                for plugin in self.core_plugins:
                    if "calibration" in plugin.command_full:
                        plugin.recreate_mdark(obs, plp, pending_json["Path"], pending_json["Binning"], pending_json["Filter"])
                        break
            ...
            # Re-reduce flat field
                ...
                for plugin in self.core_plugins:
                    if "calibration" in plugin.command_full:
                        plugin.recreate_mflat(obs, plp, pending_json["Path"], pending_json["Binning"], pending_json["Filter"])
                        break
    ...
    @staticmethod
    def run_plugins_single(obs, plp, file, called_from):
        # Collect all plugins
        ...
        for plugin in loaded_plugins.plugins:
            ...
            if (called_from == "pending" and plugin.rerun_on_pending) or \
               (called_from == "astrometry" and plugin.rerun_on_astrometry):
                ps.module(f"(Re)running external plugin: {plugin.title}")
                plugin.on_run_single(obs, plp, file)
                ps.module(f"{plugin.title} (re-)executed")
